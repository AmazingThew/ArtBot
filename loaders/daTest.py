...
CLIENT_ID     = '3823'
CLIENT_SECRET = 'ebdbe992445cd52b086030439f710fb3'
REDIRECT_URI  = 'http://localhost:58008/da_callback'
state         = str(uuid4())
preexistingToken = ''
...
@app.route('/')
def homepage():
    return '<a href="{0}">Request new token</a><br/><a href="{1}">Use existing token</a>'.format(
...
def make_authorization_url():
    ...
    return "https://www.deviantart.com/oauth2/authorize?" + parse.urlencode(params)

def makeFeedUrl(token):
    return 'https://www.deviantart.com/api/v1/oauth2/feed/home?' + parse.urlEncode({'access_token' : token})
...
def loadFeed(token):
    response = requests.get(makeFeedUrl(token))
    rj = response.json()
    return parse(rj)


def parse(responseDict):
    deviations = [deviation for item in responseDict['items'] for deviation in item['deviations'] if item['type'] == 'deviation_submitted']
    images = ['<img src=' + d['content']['src'] + '></img>' for d in deviations]
    print(images)
